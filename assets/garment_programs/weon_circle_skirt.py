...
class CircleArcPanel(pyg.Panel):
    ...
    @staticmethod
    def from_w_length_suns(name, length, top_width, bottom_width, **kwargs):
        radius = length / (1 - top_width / bottom_width)
        angle = bottom_width / radius

        return CircleArcPanel(name=name, top_rad=radius - length, length=length, angle=angle, **kwargs)
...
class SkirtCircle(StackableSkirtComponent):
    ...
    def __init__(self, body, design, tag='', length=None, rise=None, slit=True, asymm=False, min_len=5, **kwargs) -> None:
        ...
        bottom_width = design['bottom_width']['v']
        length = max(design['length']['v'], min_len)
        waist = design['waist_width']['v']
